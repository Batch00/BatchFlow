...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM transactions WHERE user_id = uid LIMIT 1) THEN
    RAISE EXCEPTION 'Demo user already has transactions. Aborting to prevent duplicate seed.';
  END IF;
